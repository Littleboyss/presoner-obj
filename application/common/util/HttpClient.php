...
class HttpClient {
    ...
    // Basic authorization publiciables
    public $username;
    public $password;
    // Response publics
    public $status;
    ...
    function doRequest() {
		...
		if (!$fp = @fsockopen($this->host, $this->port, $errno, $errstr, 3)) {
		    // Set error message
            switch($errno) {
				case -3:
        ...
        fwrite($fp, $request);
    	// Reset all the publiciables that should not persist between requests
    	$this->headers = array();
    	$this->content = '';
    	$this->errormsg = '';
        ...
        if (isset($this->headers['content-encoding']) && $this->headers['content-encoding'] == 'gzip') {
            ...
            $this->content =  \gzinflate($this->content); //\gzdecode($this->content) ;
        }
    ...
    function buildRequest() {
        ...
        $headers[] = "Host: ".str_replace("ssl://","",$this->host)."";
    ...
    static function quickGet($url) {
        $bits = parse_url($url);
        $host = $bits['host'];
        $port = isset($bits['port']) ? $bits['port'] : ($bits['scheme'] == "https"?443:80);
        $path = isset($bits['path']) ? $bits['path'] : '/';
        if (isset($bits['query'])) {
            $path .= '?'.$bits['query'];
        }
        $client = new HttpClient(($bits['scheme'] == "https")?'ssl://'.$host:$host, $port);
		//$client->debug = true;
        if (!$client->get($path)) {
            return false;
        } else {
   ...
   static function quickPost($url, $data) {
        ...
        if (!$client->post($path, $data)) {
			$client->writelog($url.':'.$client->getError());
            return false;
        } else {
            return $client->getContent();
        }
	...
	public function getLogName(){
		$filedir = RUNTIME_PATH . 'log' . DS."Data/Paylog/httperror/";
		
		$filename = $filedir."relog".date("Ymd").".txt";
		if(!is_dir($filedir)){
